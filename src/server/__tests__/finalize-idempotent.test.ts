...
describe("Finalize idempotent behavior", () => {
  beforeEach(() => {
    (
  ...
  it("second finalize returns same charterId without error", async () => {
    // First call: transitions from DRAFT -> SUBMITTED and sets charterId
    const first = await finalizeHandler(
      new Request("http://t/", {
        method: "POST",
        body: JSON.stringify({
          media: {
        ...
        headers: { "x-request-id": "fin-idem-1" },
      }),
      { params: { id: "draft-1" } }
    );
    ...
    expect(json1.requestId).toBe("fin-idem-1");

    // Mock draft now SUBMITTED with charterId
    (
      prisma.charterDraft.findUnique as unknown as {
      ...
      userId: "user-1",
      status: "SUBMITTED",
      charterId: json1.charterId,
      version: 1,
      updatedAt: new Date().toISOString(),
      data: baseDraft(),
    });

    const second = await finalizeHandler(
      new Request("http://t/", {
        method: "POST",
        body: JSON.stringify({
          media: {
        ...
        headers: { "x-request-id": "fin-idem-2" },
      }),
      { params: { id: "draft-1" } }
    );
    ...
    expect(json2.charterId).toBe(json1.charterId);
    expect(json2.requestId).toBe("fin-idem-2");
  });
});
