...
describe('media batch analytics', () => {
  ...
  it('emits media_batch_complete after all items finished', () => {
    emitCharterFormEvent({ type: 'media_upload_start', kind: 'photo', pending: 3 });
    ...
    const evt = batch[0] as Extract<AnalyticsEvent, { type: 'media_batch_complete' }>;
  ...
  it('separate batches do not interfere', () => {
    emitCharterFormEvent({ type: 'media_upload_start', kind: 'photo', pending: 1 });
    emitCharterFormEvent({ type: 'media_upload_start', kind: 'video', pending: 2 });
