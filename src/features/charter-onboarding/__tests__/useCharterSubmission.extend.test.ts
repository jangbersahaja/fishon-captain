...
describe("useCharterSubmission (extended)", () => {
  ...
  const baseSetup = (opts: {
    ...
    ).result.current as unknown as ReturnType<typeof useForm<CharterFormValues>>;
    ...
    global.fetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url === "/api/blob/upload") {
        ...
        return new Response(
          JSON.stringify({ key: `up-${fetchCalls.length}.jpg`, url: `https://cdn/x/${fetchCalls.length}.jpg` }),
      ...
      if (url.match(/\/api\/charter-drafts\/.*$/) && init?.method === "PATCH") {
        return new Response(JSON.stringify({ draft: { id: opts.draftId || "d-ext", version: (opts.patchedVersion ?? 5) + 1 } }), {
          status: 200,
          headers: { "Content-Type": "application/json" },
        });
      }
    ...
    return { hook, form, fetchCalls, push, saveServerDraftSnapshot, clearDraft, initializeDraftState };
  };

  it("sends x-draft-version header from patched snapshot version", async () => {
    const { hook, form, fetchCalls, saveServerDraftSnapshot } = baseSetup({ patchedVersion: 42, draftId: "draft-v" });
    ...
    expect(finalizeCall?.init?.headers).toMatchObject({ "x-draft-version": "42" });
  });

  it("includes avatar in finalize payload when provided", async () => {
    const { hook, form, fetchCalls } = baseSetup({ patchedVersion: 7, draftId: "draft-avatar" });
    act(() => {
      const mk = (n: string) => new File(["x"], n, { type: "image/jpeg" });
      form.setValue("photos", [mk("a.jpg"), mk("b.jpg"), mk("c.jpg")]);
      form.setValue("operator.avatar", new File(["bin"], "avatar.jpg", { type: "image/jpeg" }));
    });
    await act(async () => {
      await hook.result.current.triggerSubmit();
    });
  ...
  (global.fetch as unknown as ReturnType<typeof vi.fn>).mockImplementation(async (input: RequestInfo | URL, init?: RequestInit) => {
      const url = typeof input === "string" ? input : input.toString();
      ...
      if (url.match(/\/api\/charter-drafts\/.*$/) && init?.method === "PATCH") {
        return new Response(JSON.stringify({ draft: { id: "d", version: 3 } }), {
          status: 200,
          headers: { "Content-Type": "application/json" },
        });
      }
    ...
    expect(hook.result.current.submitState?.message).toMatch(/fix highlighted/i);
  });
  ...
  (global.fetch as unknown as ReturnType<typeof vi.fn> | undefined)?.mockReset?.();
  ...
  expect(hook.result.current.submitState?.type).toBe("error");
  expect(hook.result.current.submitState?.message.toLowerCase()).toMatch(/save failed|save error/);
  });
});
