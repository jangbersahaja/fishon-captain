"use client";
// SpeciesSelector: Tabbed selector that stores species ids. Includes per-category counts,
// search filtering, and dual-line chips (English + local name). Integrate with RHF by
// passing current array and onChange handler.
import {
  ALL_SPECIES,
  SPECIES_BY_CATEGORY,
  SPECIES_CATEGORIES,
...
type SpeciesSelectorProps = {
  value?: string[]; // array of species ids
  onChange: (next: string[]) => void;
  maxVisiblePerCategory?: number; // optional limit per tab (not enforced yet)
  activeTab?: SpeciesCategory; // controlled active tab
  onActiveTabChange?: (tab: SpeciesCategory) => void;
  maxSelected?: number; // hard limit of selections
};
...
export function SpeciesSelector({
  ...
  function toggle(id: string) {
    ...
    if (set.has(id)) {
      ...
      if (value.length >= maxSelected) return; // enforce limit
      set.add(id);
    }
