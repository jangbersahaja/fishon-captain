...
export const ToastProvider: React.FC<ProviderProps> = ({
  ...
  const push: ToastContextValue['push'] = useCallback((partial) => {
    const id = partial.id || `t_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;
    setToasts(prev => {
      // replacement logic
      const existingIdx = prev.findIndex(t => t.id === id);
      if (existingIdx !== -1 && (partial.replace || prev[existingIdx].type === 'progress')) {
        const next: Toast = {
          ...prev[existingIdx],
          ...partial,
          id,
      ...
      if (toast.type === 'success') {
        const last = prev[0];
        if (last && last.type === 'success' && last.message === toast.message && Date.now() - last.createdAt < 1200) {
  ...
  const registerBottomAnchor: ToastContextValue['registerBottomAnchor'] = useCallback((id, getHeight) => {
  ...
  return (
    <ToastContext.Provider value={{ push, dismiss, update, registerBottomAnchor }}>
      {children}
      {mounted && createPortal(
        <ToastViewport toasts={toasts} dismiss={dismiss} dynamicOffset={offset} />,
        document.body
      )}
...
const ToastViewport: React.FC<ViewportProps> = ({ toasts, dismiss, dynamicOffset }) => {
  return (
    <div
      className="pointer-events-none fixed left-1/2 z-[60] flex w-full max-w-[380px] -translate-x-1/2 flex-col gap-2 px-4"
      style={{ bottom: `calc(env(safe-area-inset-bottom, 0px) + ${dynamicOffset}px)` }}
      aria-live="polite"
      role="region"
...
const ToastItem: React.FC<{ toast: Toast; onDismiss: () => void }> = ({ toast, onDismiss }) => {
  ...
  return (
    ...
    >
      ...
      {type === 'progress' && (
        <span className="ml-auto animate-pulse text-xs text-white/70">Saving</span>
      )}
      {/* Animation classes (basic) */}
      <style jsx>{`
        div[data-toast-type] { opacity: 0; transform: translateY(12px); animation: toast-in 200ms ease-out forwards; }
        @media (prefers-reduced-motion: reduce) { div[data-toast-type] { transform: none; animation: fade-in 160ms linear forwards; } }
